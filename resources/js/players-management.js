...
$(function () {
  ...
  var dt_user_table = $('.datatables-players'),
    select2 = $('.select2'),
  ...
  $(document).on('click', '.delete-record', function () {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            // Solicitud AJAX para eliminar El player
            $.ajax({
                url: deleteUrl,
                type: 'POST',
                data: {
                ...
                success: function (response) {
                    ...
                    Swal.fire({
                        icon: 'success',
                        title: 'Eliminado!',
                        text: 'El player ha sido eliminada.',
                        customClass: {
                ...
                error: function (error) {
                    ...
                    Swal.fire({
                        icon: 'error',
                        title: 'Error',
                        text: 'Hubo un problema al eliminar El player. Intenta de nuevo.',
                        customClass: {
            ...
            Swal.fire({
                title: 'Cancelado',
                text: 'El player no fue eliminada.',
                icon: 'error',
                customClass: {
                    confirmButton: 'btn btn-success'
                }
  ...
  $(document).on('click', '.edit-record', function () {
    $(".offcanvas-title").html("Editar player");
...
$(document).on('click', '#addNewTematicaButton', function () {
    ...
    $(".offcanvas-title").html("Agregar nuevo player");
    $('#userForm').attr('action', '/players');
